...
namespace WebApi.ActionFilters
{
    ...
    public class GlobalExceptionAttribute : ExceptionFilterAttribute
    {
        public override void OnException(HttpActionExecutedContext context)
        {
            ...
            else if (exceptionType == typeof(ExternalApiException))
            {
                var externalApiException = context.Exception as ExternalApiException;
                if (externalApiException != null)
                    throw new HttpResponseException(context.Request.CreateResponse(externalApiException.HttpStatus, new ServiceStatus() { StatusCode = externalApiException.ErrorCode, StatusMessage = externalApiException.ErrorDescription, ReasonPhrase = externalApiException.ReasonPhrase }));
            }
            else
            {
